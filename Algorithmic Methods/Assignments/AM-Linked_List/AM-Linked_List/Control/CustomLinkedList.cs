    ...
    public T this[int index]
    {
        get
        {
            lock (this)
            {
                ...
                {
                    throw new InvalidOperationException("Index must be less than the size of the list and non-negative.");
                }
                
                Node<T>? currentNode = FirstNode;

                // Iterates through the list and finds the node at the given index
                for (int i = 0; i < index; i++)
                {
                    // currentNode will never be null as Index should always be within bounds
                    currentNode = currentNode!.Next;
                }
                
                return currentNode!.Item ?? throw new InvalidOperationException($"The item at index {index} does not exist in the list.");
            }
        }
    }
